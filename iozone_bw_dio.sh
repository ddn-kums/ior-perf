...
run_iozone_seq () {
	...
	local RESULT_FILE_WR="/work/kums/benchmarks/iozone/results/$VERSION/iozone_bw_dio_WR_`date +%F-%T`.txt"
  	...
  	do
		...
		do
			...
			do 
				...
				echo "$BENCHMARK -r '$io_sz'k -s $FSIZE -i 0 -i 1 -e -I -c -C -+u -+m $MACH_FILE -+n -t $np" >> $RESULT_FILE_WR
				$BENCHMARK -r "$io_sz"k -s $FSIZE -i 0 -i 1 -e -I -c -C -+u -+m $MACH_FILE -+n -t $np 2>&1 | tee -a $RESULT_FILE_WR
